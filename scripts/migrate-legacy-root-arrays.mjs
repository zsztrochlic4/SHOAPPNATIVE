...
async function main() {
  if (!process.env.GOOGLE_APPLICATION_CREDENTIALS) {
    console.error('✖ Set GOOGLE_APPLICATION_CREDENTIALS to a service-account JSON.'); process.exit(2)
  }
  ...
  for (const snap of users) {
    if (!snap.exists) { console.log(`  ${snap.id}: not found`); continue }
    const uid = snap.id
    ...
    for (const c of present) {
      ...
      for (const e of data[c]) {
        const id = KEY_OF[c](e)
        if (id == null || id === '') { (skipped[c] ??= []).push(e); continue }
        plan[c].push({ id: String(id), data: clean(e) })
      }
    }
    const counts = present.map((c) => `${c}:${plan[c].length}${skipped[c] ? `(+${skipped[c].length} skipped)` : ''}`).join('  ')
    console.log(`\n  ${uid}\n    migrate → ${counts}`)

    const anySkipped = Object.keys(skipped).length > 0
    if (anySkipped) console.log(`    ⚠ some elements lack a stable id/dateKey and will NOT be moved or deleted (kept in root).`)
    ...
    for (const c of present) {
      ...
      for (const part of chunk(ops, BATCH)) {
        const b = db.batch()
        for (const { id, data: doc } of part) b.set(db.collection('users').doc(uid).collection(c).doc(id), doc, { merge: true })
        await b.commit()
      }
    }
    ...
    for (const c of present) {
      ...
      if (got < plan[c].length) { verifyOk = false; console.log(`    ✖ verify ${c}: sub=${got} < expected ${plan[c].length} — NOT deleting root arrays`) }
    }
    if (!verifyOk) { console.log(`    ✖ aborting root cleanup for ${uid} (data preserved in root).`); continue }
    ...
    if (Object.keys(del).length) { await db.collection('users').doc(uid).update(del); console.log(`    ✓ root arrays removed: ${Object.keys(del).join(', ')}`) }
  }
