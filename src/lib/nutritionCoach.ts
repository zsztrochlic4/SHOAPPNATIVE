...
export function reviewDay(text: string, goal: Goal): DayReview {
  ...
  if (proteinSources >= 1) highlights.push(`Protein from ${list(proteinFoods.slice(0, 3))}, which supports recovery and keeps you full.`)
...
function goalNextTip(goal: Goal): string {
  switch (goal) {
    case 'build-muscle': return 'Strong base. To push muscle growth, make sure you\'re eating enough overall, with an extra carb portion around training to help.'
