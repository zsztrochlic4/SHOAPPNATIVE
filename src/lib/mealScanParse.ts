/**
 * Pure parsing + validation for the AI meal scan (no firebase/RN imports, so it
 * can be unit-tested). `mealScan.ts` calls Gemini vision and hands the raw model
 * text here; this module gates food/non-food and clamps every number into a sane
 * range, so a malformed or over-confident model reply can never reach the UI.
 *
 * Design note (Production Readiness plan §4.4 / Blocker #1): estimates from a
 * single photo are inherently rough, so the model is asked for a calorie RANGE
 * and a confidence level, never a single confident number.
 */
...
/** The instruction sent with the photo. Kept here so the shape and the parser stay in sync. */
...
/** Pull the first {...} block out of a reply (models sometimes add fences/prose despite instructions). */
...
/**
 * Parse a raw model reply into a validated MealAnalysis, or throw if it is not
 * usable JSON. Every field is type-checked and clamped; unknown enum values fall
 * back to the safest option (low confidence / moderation).
 */
