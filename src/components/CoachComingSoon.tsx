...
export function CoachComingSoon() {
  ...
  return (
    <View className="items-center rounded-3xl border border-white/8 bg-ink-800 px-6 py-10">
      ...
      <Text className="mt-5 text-center text-xl font-extrabold text-white">{t('Coach is coming soon')}</Text>
      <Text className="mt-2.5 max-w-[300px] text-center text-[14px] leading-6 text-secondary">{t('Your coach is being reviewed by a qualified professional before it goes live. We’ll switch it on here once it’s ready.')}</Text>
    </View>
  )
}
