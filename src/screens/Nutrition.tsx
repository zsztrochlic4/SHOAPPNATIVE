...
/**
 * Map the design's dark-theme accent hex (stored on the static guide data below)
 * to the LIVE palette, so the Nutrition screen flips to the light theme like the
 * rest of the app instead of staying neon on cream. Unknown values pass through.
 */
type Colors = ReturnType<typeof useColors>
function tint(hex: string, c: Colors): string {
  switch (hex.toLowerCase()) {
    case '#7ed957': return c.brand400
    case '#9fe264': return c.brand300
    case '#3b82f6': return c.accentBlue
    case '#f5a524': return c.accentOrange
    case '#8b5cf6':
    case '#a98bf5': return c.accentPurple
...
function useScrollPad() {
  ...
  return { paddingBottom: insets.bottom + TAB_CLEARANCE }
}
...
/* A green "viewfinder" framing guide shown before the camera opens, so the user
 * knows to centre and fill the frame with their meal — the OS camera itself can't
 * be overlaid, so this sets the framing up front. Four brand corner-brackets
 * around a short hint. */
function ScanFrameGuide() {
  const c = useColors()
  const B = 28 // corner-bracket arm length
  const green = c.brand400
  const base = { position: 'absolute' as const, width: B, height: B, borderColor: green }
  return (
    <View
      accessibilityLabel="Framing guide: centre your meal in the frame"
      className="mt-3.5 items-center justify-center overflow-hidden rounded-[18px]"
      style={{ height: 138, backgroundColor: alpha(green, 0.05), borderWidth: 1, borderColor: alpha(green, 0.18) }}
    >
      <Text style={{ fontSize: 30 }}>🍽️</Text>
      <Text className="mt-1.5 text-[12.5px] font-extrabold" style={{ color: green }}>Centre your meal in the frame</Text>
      <Text className="mt-0.5 text-[11px]" style={{ color: alpha(c.fg, 0.5) }}>Fill the box · good light · straight-down works best</Text>
      <View style={{ ...base, top: 12, left: 12, borderTopWidth: 3, borderLeftWidth: 3, borderTopLeftRadius: 13 }} />
      <View style={{ ...base, top: 12, right: 12, borderTopWidth: 3, borderRightWidth: 3, borderTopRightRadius: 13 }} />
      <View style={{ ...base, bottom: 12, left: 12, borderBottomWidth: 3, borderLeftWidth: 3, borderBottomLeftRadius: 13 }} />
      <View style={{ ...base, bottom: 12, right: 12, borderBottomWidth: 3, borderRightWidth: 3, borderBottomRightRadius: 13 }} />
    </View>
  )
}
          ...
          {NUTRITION_TAGS.map((t) => {
            ...
            return (
              <Pressable
                ...
                style={{ width: '48%', paddingVertical: 13, paddingHorizontal: 12, gap: 9, backgroundColor: on ? alpha(tint, 0.16) : alpha(c.fg, 0.05), boxShadow: on ? `inset 0 0 0 1.5px ${alpha(tint, 0.55)}` : undefined }}
                className="flex-row items-center rounded-[14px] active:opacity-80"
              >
                <Text className="text-[19px] leading-none">{t.emoji}</Text>
                <Text className="min-w-0 flex-1 text-[13px] font-semibold leading-[1.15]" style={{ color: on ? tint : alpha(c.fg, 0.75) }}>{t.label}</Text>
...
function GuideTab() {
  ...
  return (
    ...
    >
      ...
      <View className="flex-row items-center gap-[11px]" style={[{ marginHorizontal: -20, paddingHorizontal: 18, paddingVertical: 8, backgroundColor: c.ink900, borderBottomWidth: 1, borderBottomColor: alpha(c.fg, 0.08) }, WEB_STICKY]}>
      ...
      <View className="mt-3.5 flex-row items-center justify-center gap-2.5">
        ...
        <Text className="text-[13px] font-semibold text-white/60">Add a small amount of <Text onPress={() => { setPlate(3); setPlateEx(false) }} className="font-bold" style={{ color: tint(active.slice === 'fat' ? '#a98bf5' : '#8B5CF6', c) }}>healthy fat</Text> on the side</Text>
      </View>
...
function BalancedPlate({ selected, onSelect }: { selected: PlateSlice['slice']; onSelect: (i: number) => void }) {
  ...
  const stroke = (key: string) => (selected === key ? alpha(c.fg, 0.75) : c.ink900)
  ...
  return (
    <View style={{ width: 236, height: 236, alignSelf: 'center' }}>
      <Svg viewBox="0 0 200 200" width={236} height={236}>
        ...
        <Circle cx="100" cy="100" r="27" fill={c.ink900} />
      </Svg>
    </View>
  )
}
        ...
        <Pressable onPress={() => {}} className="w-full max-w-[360px] overflow-hidden rounded-[26px] border border-white/10 bg-ink-800" style={{ maxHeight: '88%' }}>
          {budget ? (
            <View className="h-[158px]">
              ...
              <LinearGradient colors={['transparent', alpha(c.ink900, 0.4), c.ink900]} style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0 }} />
              ...
              <View className="absolute inset-x-4 bottom-3">
                ...
                <View className="mt-1 flex-row items-center gap-1.5"><Clock size={13} color={c.brand300} /><Text className="text-[12.5px] font-bold text-brand-300">{budget.timeDisplay ?? `${budget.minutes} min`} · {budget.category} · serves {budget.serves}</Text></View>
              </View>
            </View>
          ) : (
