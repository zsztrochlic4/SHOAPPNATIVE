...
function buildPrompt(text: string, ctx: CoachContext, recent: string[]): string {
  ...
  return [
    ...
    '- meal_plan: a request for a personalised FULL FOOD PLAN or exact per-macro targets (e.g. "build me a meal plan", "give me my exact macros for the day"). NOT meal_plan: a single general nutrition question ("how much protein should I aim for", "how much water should I drink", "should I eat before training") — that is normal coaching. A TRAINING split or workout program (push/pull/legs, upper/lower) is also normal coaching, NOT a meal_plan.',
