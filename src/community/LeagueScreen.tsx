      ...
      <View className="mb-4 flex-row gap-2 rounded-2xl bg-ink-700 p-1">
        {(['league', 'global'] as const).map((v) => {
          ...
          return (
            ...
            >
              <Text className={`text-[13px] font-bold ${active ? 'text-black' : 'text-secondary'}`}>{v === 'league' ? t('My league') : t('Global streaks')}</Text>
            </Pressable>
          )
        })}
      </View>
...
function LeagueError({ onRetry }: { onRetry: () => void }) {
  ...
  return (
    <View className="mt-6 items-center rounded-2xl border border-dashed border-white/15 px-6 py-12">
      <Text className="font-bold text-white">{t("Couldn't load your league")}</Text>
    ...
    <View className="mb-3 rounded-2xl border p-4" style={{ borderColor: `${accent}55`, backgroundColor: `${accent}14` }}>
      <View className="flex-row items-center gap-3">
        ...
        <View className="min-w-0 flex-1">
          <Text className="font-bold text-white">{t('Your standing is under review')}</Text>
          <Text className="mt-0.5 text-[13px] leading-snug text-secondary">
            {held
              ? t("Something in this week's activity looked unusual, so your rank is paused while we take a closer look. If this looks wrong, ask us to check again.")
              : t("Your rank is paused while we double-check this week's activity. It usually clears on its own, so keep logging as normal.")}
          </Text>
        </View>
      </View>
      {held && (
        ...
        >
          <Text className="text-[14px] font-bold text-white/90">{t('Ask for another look')}</Text>
...
function AppealSheet({ open, onClose, onDone }: { open: boolean; onClose: () => void; onDone: () => void }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      thud()
      toast(res.status === 'ok' ? t("You're back in the running") : t("Thanks, we'll take another look"))
      setNote('')
      onClose()
      onDone()
    } catch {
  ...
  return (
    <Sheet open={open} onClose={onClose} title={t('Ask for another look')}>
      <Text className="text-[13px] leading-snug text-secondary">
        {t("If your recent training was genuine, tell us anything that helps and we'll re-check your standing. This won't lower your rank.")}
      </Text>
      <View className="mt-3 rounded-2xl border border-white/12 bg-ink-700 px-3.5 py-3">
        <TextInput
          ...
          multiline
          maxLength={500}
          placeholder={t('Add a note (optional), e.g. I trained twice on Saturday')}
      ...
      >
        {submitting ? <ActivityIndicator size="small" color="#000" /> : <Text className="text-[15px] font-bold text-black">{t('Send appeal')}</Text>}
      </PressableScale>
    </Sheet>
  )
}
...
function LeagueFillingCard({ count }: { count: number }) {
  ...
  const invite = async () => {
    const res = await shareText(
      t("I'm competing on StrengthHub. Join my league and let's climb together."),
      'StrengthHub',
    )
    if (res === 'copied') { tick(); toast(t('Invite copied to clipboard')) }
    else if (res === 'failed') toast(t("Couldn't open share"))
  }
  return (
    <View className="mt-4 rounded-2xl border border-dashed border-white/15 bg-ink-800 p-4">
      <View className="flex-row items-center gap-3">
        ...
        <View className="min-w-0 flex-1">
          <Text className="font-bold text-white">{t('Your league is filling up')}</Text>
          <Text className="mt-0.5 text-[13px] leading-snug text-secondary">
            {count <= 1
              ? t("You're first in. New lifters land here every day. Invite a friend and climb together.")
              : t('{count} lifters so far. More join your league as StrengthHub grows.', { count })}
          </Text>
        </View>
      </View>
      <PressableScale
      ...
      >
        ...
        <Text className="text-[14px] font-bold text-black">{t('Invite friends')}</Text>
      </PressableScale>
    </View>
  )
}
    ...
    <View className="overflow-hidden rounded-2xl border p-4" style={{ borderColor: `${tier.color}66`, backgroundColor: `${tier.color}14` }}>
      <View className="flex-row items-center gap-3">
        ...
        <View className="flex-1">
          <Text className="text-[12px] font-semibold uppercase tracking-wide" style={{ color: tier.color }}>{t('{tier} League', { tier: tier.name })}</Text>
          <View className="flex-row items-end gap-1.5">
            <Text className="text-[30px] font-black leading-tight text-white">#{rank}</Text>
            <Text className="mb-1.5 text-[13px] font-semibold text-secondary">{t('of {cohort}', { cohort })}</Text>
          </View>
        </View>
        <View className="items-end">
          <View className="flex-row items-center gap-1">
            <Clock size={12} color="rgba(255,255,255,0.45)" />
            <Text className="text-[12px] font-bold text-secondary">{t('{days}d left', { days })}</Text>
          </View>
      ...
      {zone === 'promote' ? (
        <Text className="mt-3 text-[13px] font-semibold" style={{ color: brand[400] }}>
          {t('In the promotion zone. Hold it to reach')} <Text style={{ color: nextTier.color }}>{nextTier.name}</Text>!
        </Text>
      ) : zone === 'demote' ? (
        <Text className="mt-3 text-[13px] font-semibold" style={{ color: '#F5A524' }}>{t('In the drop zone. One good session moves you up.')}</Text>
      ) : canPromote ? (
        <Text className="mt-3 text-[13px] font-semibold text-secondary">
          {t('Reach the top {cutoff} to promote to', { cutoff })} <Text style={{ color: nextTier.color, fontWeight: '700' }}>{nextTier.name}</Text>.
        </Text>
      ) : (
        <Text className="mt-3 text-[13px] font-semibold text-secondary">{t("You're in the top tier. Hold your rank to stay in {tier}.", { tier: tier.name })}</Text>
      )}
      <Pressable onPress={onHow} accessibilityRole="button" accessibilityLabel="How leagues work" className="mt-2 flex-row items-center gap-1 active:opacity-70">
        ...
        <Text className="text-[12px] font-semibold text-secondary">{t('How leagues work')}</Text>
      </Pressable>
    </View>
  )
}
...
function StreakCard() {
  ...
  const freezeStreak = () => {
    ...
    toast(t('Streak frozen for today'))
  }
  ...
  return (
    <View className="mt-3 rounded-2xl border border-white/8 bg-ink-800 p-4">
      <View className="flex-row items-center gap-3">
        ...
        <View className="flex-1">
          <Text className="text-[13px] font-bold text-white">{t('{streak}-day streak', { streak: me.streakCurrent })}</Text>
          <Text className="text-[12px] text-secondary">{t('Best {best} · rest days & freezes keep it alive', { best: me.streakBest })}</Text>
        </View>
      ...
      <View className="mt-3">
        {restingToday ? (
          ...
          <View className="flex-row items-center justify-center gap-1.5 rounded-xl bg-brand-400/20 py-2.5">
            ...
            <Text className="text-[13px] font-bold text-brand-300">{t('Resting today')}</Text>
          </View>
        ) : (
          ...
          >
            ...
            <Text className="text-[13px] font-bold" style={{ color: '#6AD1E3' }}>{tokens > 0 ? t('Freeze streak') : t('No freezes left')}</Text>
          </PressableScale>
        )}
      </View>
    </View>
...
function LeagueStandings({ rows, tier }: { rows: LeagueRow[]; tier: ReturnType<typeof tierOf> }) {
      ...
      items.push(
        <View key={`z-${r.zone}`} className="mb-1 mt-3 flex-row items-center gap-1.5">
          ...
          <Text className="text-[11px] font-bold uppercase tracking-wide" style={{ color: z.color }}>
            {t(z.label)}{r.zone === 'safe' ? ` ${t('in {tier}', { tier: tier.name })}` : ''}
          </Text>
        </View>,
      )
      prevZone = r.zone
  ...
  return (
    <View className="mt-4">
      <Text className="mb-1 section-title">{t("This month's standings")}</Text>
      {items}
    </View>
  )
}
...
function StandingRow({ row }: { row: LeagueRow }) {
  ...
  return (
    ...
    >
      ...
      <Text numberOfLines={1} className={`flex-1 font-bold leading-tight ${you ? 'text-brand-300' : 'text-white'}`}>@{row.username}{you ? ` ${t('(You)')}` : ''}</Text>
      <View className="items-end">
...
function HowLeaguesSheet({ open, onClose }: { open: boolean; onClose: () => void }) {
  const t = useT()
  return (
    <Sheet open={open} onClose={onClose} title={t('How leagues work')}>
      <View className="gap-3">
        <HowRow n="1" title={t('Compete each month')} body={t("You're placed in a league with others at your level, up to ~25 as it fills. Your score is your dashboard odometer, how consistently you hit your goals. It's about showing up, not how much you lift.")} />
        <HowRow n="2" title={t('Climb the ladder')} body={t('On the first Monday of every month the league resets. Finish in the top 30% and you promote to the next tier: Bronze, Silver, Gold, Platinum, Diamond. The higher you go, the tougher the climb.')} />
        <HowRow n="3" title={t('Mind the drop zone')} body={t("Finish in the bottom 30% and you slip down a tier. It's easy to climb straight back, a couple of good days does it.")} />
        <HowRow n="4" title={t('Streaks are forgiving')} body={t('A planned rest day or a freeze token keeps your streak alive through an off day. You get 2 fresh freezes at each monthly reset, no guilt, no all-or-nothing.')} />
      </View>
      <View className="mt-4 flex-row flex-wrap gap-2">
        {TIERS.map((v) => (
          <View key={v.key} className="flex-row items-center gap-1.5 rounded-full px-2.5 py-1" style={{ backgroundColor: `${v.color}1f` }}>
            <View className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: v.color }} />
            <Text className="text-[11px] font-bold text-white/80">{v.name}</Text>
          </View>
        ))}
      </View>
    </Sheet>
