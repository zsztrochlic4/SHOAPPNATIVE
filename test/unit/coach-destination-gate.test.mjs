 ...
 * misfires the 2000-prompt eval exposed: a card whose action was removed from the app (open_budget_eats),
 * and a technique-guide navigation whose exercise resolves to no real lift. The gate drops the CARD only;
 * the text answer is unaffected (asserted at the coach-orchestration layer, not here).
 */
...
test('drops a card whose action was removed from the app (open_budget_eats), on any turn', () => {
  for (const m of ['show me cheap student meals', 'How do I set up for a barbell back squat?', 'open budget eats']) {
    assert.equal(proposalDestinationIssue(action('open_budget_eats'), m), 'action_unknown', `should drop for: ${m}`)
  }
})
